...
static int set_path_mode_bits(const char *path, mofs_uint16_t perm_bits)
{
    int          ret       = 0;
    int          inode_num = -1;
    mofs_inode_t inode;
    ...
    inode.i_mode = (mofs_uint16_t)((inode.i_mode & (MOFS_FTYPE_DIR | MOFS_FTYPE_REG)) | (perm_bits & 07777U));
...
static void test_traverse_denied_without_execute_on_parent(void **state)
{
    ...
    assert_int_equal(mofs_mkdir("/locked", (mofs_mode_t)(MOFS_FTYPE_DIR | 0700U)), 0);
    assert_int_equal(mofs_mkdir("/locked/open", (mofs_mode_t)(MOFS_FTYPE_DIR | 0777U)), 0);

    assert_int_equal(mofs_set_caller_user((mofs_uid_t)1001, (mofs_gid_t)1001, getpid()), 0);

    ret = mofs_create_core("/locked/open/file.txt", 0644U, &inode_num);
    assert_int_equal(ret, MOFS_EACCES);

    ret = mofs_mkdir_core("/locked/open/sub", (mofs_mode_t)(MOFS_FTYPE_DIR | 0777U));
    assert_int_equal(ret, MOFS_EACCES);
...
static void test_create_denied_without_search_on_parent(void **state)
{
    ...
    assert_int_equal(mofs_set_caller_user((mofs_uid_t)0, (mofs_gid_t)0, getpid()), 0);
    assert_int_equal(mofs_mkdir("/nowx", (mofs_mode_t)(MOFS_FTYPE_DIR | 0732U)), 0);
    assert_int_equal(mofs_mkdir("/nowx/child", (mofs_mode_t)(MOFS_FTYPE_DIR | 0777U)), 0);
...
static void test_opendir_denied_on_unreadable_directory(void **state)
{
    ...
    assert_int_equal(mofs_set_caller_user((mofs_uid_t)0, (mofs_gid_t)0, getpid()), 0);
    assert_int_equal(mofs_mkdir("/privdir", (mofs_mode_t)(MOFS_FTYPE_DIR | 0000U)), 0);
...
static void test_unlink_denied_without_parent_write(void **state)
{
    ...
    assert_int_equal(mofs_set_caller_user((mofs_uid_t)0, (mofs_gid_t)0, getpid()), 0);
    assert_int_equal(mofs_mkdir("/parent", (mofs_mode_t)(MOFS_FTYPE_DIR | 0777U)), 0);
